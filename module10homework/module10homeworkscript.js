...
class Req {
    ...
    getUserById() {
        ...
        fetch(`https://test-users-api.herokuapp.com/users/${this.inputName.value}`)
            ...
            .catch(() => this.result.innerHTML = "User doesn't exist");
    }
    ...
    successAdd(data) {
        if (data.status == 500) {
            console.log(data);
            this.result.textContent = 'Enter a valid user name and a valid age!';
        } else {
            console.log(data);
            this.result.textContent = 'User enrollment successful!';
        }
    }
    ...
    successDelete(data) {
        if (data.data === null || data.status === 500 || data.status === 404) {
            ...
            this.result.textContent = 'User with entered id does not exist!';
        } else {
            console.log(data);
            this.result.textContent = 'User deleted successfully!';
        }
    }
    ...
    successUpdate(data) {
        if (data.status === 404) {
            ...
            console.log(data);
            this.result.textContent = 'User identity changed successfully!';
        }
    }
